...
整体视觉更新：雾白背景、深灰文字、蓝灰主操作色，统一导航、表单、统计卡片和悬浮窗工具栏；保留像素品牌标记及真实色板颜色。移除持续装饰动画，提供键盘焦点提示与减少动态效果支持。登录页改为居中表单，手机端统计卡片使用两列布局，紧凑顶栏保留退出入口。本地已检查桌面生成/库存页和 390px 手机生成/登录页，页面切换及悬浮窗兼容测试通过。主题配置与 Python 文件需一起提交。
